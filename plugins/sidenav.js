class ShadowElement extends HTMLElement {
    constructor() {
        ...
        this.shadowRoot.querySelector('.hidden-overlay').style.height = `${window.innerHeight + 100}px`

        // console.log(this.sideNavElement)
    }

    toggleShadow() {
        var sideNavActive = this.shadowRoot.host.getAttribute('aria-hidden')
...
function nestedListChildEl(item, iconDown) {
    ...
    Array.from(arrayNestedList).map(el => {
        ...
        var isHaveChild = Array.from(el.children).find(elChild => elChild.classList.contains('custom-collapse-2'))


        if (isHaveChild) {

            var collapsedBtn2 = isHaveChild.previousElementSibling;
            ...
            var iconText = '';
            if (!iconDown) return;
            var minifyIcon = minify_html(iconDown, iconText);

            createIconSpan.innerHTML = minifyIcon

            collapsedBtn2.appendChild(createIconSpan)
        }
    })
}
...
export function sidebar001({
    ...
    sideNavActiveButton.addEventListener('click', function (evt) {
        ...
        if (!activeSidebar.classList.contains('active')) {
            ...
            createHiddenElement.setAttribute('aria-hidden', targetNavEl);
